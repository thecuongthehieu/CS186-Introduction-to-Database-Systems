...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        long commitLSN = this.logManager.appendToLog(new CommitTransactionLogRecord(transNum, transactionTableEntry.lastLSN));
    ...
    public long abort(long transNum) {
        // TODO(proj5): implement
        TransactionTableEntry transactionTableEntry = transactionTable.get(transNum);
        long abortLSN = this.logManager.appendToLog(new AbortTransactionLogRecord(transNum, transactionTableEntry.lastLSN));
        //this.logManager.flushToLSN(abortLSN);
    ...
    public long end(long transNum) {
        ...
        if (transactionTableEntry.transaction.getStatus().equals(Transaction.Status.ABORTING)) {
            ...
            LogRecord lastRecord = this.logManager.fetchLogRecord(transactionTableEntry.lastLSN);
            long lastLSN = lastRecord.getLSN();

            do {
                if (lastRecord.isUndoable()) {
                    Pair<LogRecord, Boolean> undoReturnedValue = lastRecord.undo(lastLSN);

                    LogRecord CLRrecord = undoReturnedValue.getFirst();
                    lastLSN = this.logManager.appendToLog(CLRrecord);

                    // flush
                    if (undoReturnedValue.getSecond()) {
                        this.logManager.flushToLSN(CLRrecord.getLSN());
                    }

                    // redo
                    CLRrecord.redo(this.diskSpaceManager, this.bufferManager);

                    transactionTableEntry.lastLSN = lastLSN;
                    if (CLRrecord.getPageNum().isPresent()) {
                        if (!dirtyPageTable.containsKey(CLRrecord.getPageNum().get())) {
                            dirtyPageTable.put(CLRrecord.getPageNum().get(), lastLSN);
                        }
                    }

                    lastRecord = CLRrecord;

                }

                if (lastRecord.getUndoNextLSN().isPresent()) {
                    lastRecord = this.logManager.fetchLogRecord(lastRecord.getUndoNextLSN().get());
                } else if (lastRecord.getPrevLSN().isPresent()){
                    lastRecord = this.logManager.fetchLogRecord(lastRecord.getPrevLSN().get());
                } else {
                    break;
                }
            } while (true);
        ...
        long endLSN = this.logManager.appendToLog(new EndTransactionLogRecord(transNum, transactionTableEntry.lastLSN));
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        if (before.length > BufferManager.EFFECTIVE_PAGE_SIZE / 2) {
            long undoOnlyLSN = this.logManager.appendToLog(new UpdatePageLogRecord(transNum, pageNum, transactionTableEntry.lastLSN, pageOffset, before, new byte[0]));
            long redoOnlyLSN = this.logManager.appendToLog(new UpdatePageLogRecord(transNum, pageNum, undoOnlyLSN, pageOffset, new byte[0], after));
            lastLSN = redoOnlyLSN;
        } else {
            long updateLSN = this.logManager.appendToLog(new UpdatePageLogRecord(transNum, pageNum, transactionTableEntry.lastLSN, pageOffset, before, after));
            lastLSN = updateLSN;
        }
    ...
    public void rollbackToSavepoint(long transNum, String name) {
        ...
        // TODO(proj5): implement
        LogRecord lastRecord = this.logManager.fetchLogRecord(transactionEntry.lastLSN);
        long lastLSN = lastRecord.getLSN();
        ...
        do {
            if (lastRecord.isUndoable()) {
                ...
                if (lastRecord.getPageNum().isPresent()) {
                    if (dirtyPageTable.containsKey(lastRecord.getPageNum().get())) {
                        if (lastRecord.getLSN() > LSN) {
            ...
            if (lastRecord.getUndoNextLSN().isPresent()) { //lastRecord = CLR
                if (lastRecord.getUndoNextLSN().get() <= LSN) {
                    return;
                }
                lastRecord = this.logManager.fetchLogRecord(lastRecord.getUndoNextLSN().get());
            } else if (lastRecord.getPrevLSN().isPresent()){ //lastRecord = ABORT,....
                if (lastRecord.getPrevLSN().get() <= LSN) {
                    return;
                }
                lastRecord = this.logManager.fetchLogRecord(lastRecord.getPrevLSN().get());
            }
