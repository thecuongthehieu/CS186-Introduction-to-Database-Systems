...
public class LockContext {
    ...
    throws InvalidLockException, DuplicateLockRequestException {
        ...
        if (this.readonly) {
            throw new UnsupportedOperationException("acquire of LockContext");
        }
        ...
        if (parentContext != null) {
            if (hasSIXAncestor(transaction)) {
                if (lockType.equals(LockType.S) || lockType.equals(LockType.IS)) {
                    throw new InvalidLockException("acquire of LockContext: hasSIXAncestor");
                }
            }

            LockType parentLockType = this.lockman.getLockType(transaction, parentContext.getResourceName());

            if (LockType.canBeParentLock(parentLockType, lockType)) {
                this.lockman.acquire(transaction, this.name, lockType);
                this.updateNumChildLocks(transaction.getTransNum(), 1);
            } else if (!(!parentLockType.equals(LockType.SIX) && LockType.substitutable(parentLockType, lockType))){
                throw new InvalidLockException("acquire of LockContext");
            }
        } else {
            this.lockman.acquire(transaction, this.name, lockType);
            this.updateNumChildLocks(transaction.getTransNum(), 1);
    ...
    private void updateNumChildLocks(long transactionNum, int num) { // num is negative if release
        if (this.parentContext() != null) {
            LockContext parrentContext = this.parentContext();
            parrentContext.numChildLocks.put(transactionNum, parrentContext.numChildLocks.getOrDefault(transactionNum, 0) + num);
        }
    }
    ...
    throws NoLockHeldException, InvalidLockException {
        ...
        for (LockContext childContext : this.children.values()) {
            LockType childType = this.lockman.getLockType(transaction, childContext.getResourceName());
            if (!childType.equals(LockType.NL)) {
                throw new InvalidLockException("release of LockContext");
            }
        }
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        ...
        if (this.lockman.checkNoLockHeldException(transaction, this.name)) {
            throw new NoLockHeldException("promote of LockContext");
        }

        if (newLockType.equals(LockType.SIX)) {

            if (hasSIXAncestor(transaction)) {
                throw new InvalidLockException("promote of LockContext");
            }

            LockType oldLockType = this.lockman.getLockType(transaction, this.name);
            if (LockType.substitutable(newLockType, oldLockType)) {
                List<ResourceName> releaseLocks = this.sisDescendants(transaction);

                this.updateNumChildsLocksSISDescendants(transaction, -1);

                releaseLocks.add(this.name);

                this.lockman.acquireAndRelease(transaction, this.name, newLockType, releaseLocks); //add to queue
    ...
    private void updateNumChildsLocksSISDescendants(TransactionContext transaction, int num) { // DFS
        for (LockContext descendantContext : this.children.values()) {
            LockType currDescendantLockType = this.lockman.getLockType(transaction, descendantContext.getResourceName());
            if (currDescendantLockType.equals(LockType.S) || currDescendantLockType.equals(LockType.IS)) {
                descendantContext.updateNumChildLocks(transaction.getTransNum(), num);
            }

            descendantContext.updateNumChildsLocksSISDescendants(transaction, num);
        }
    }
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        if (currLockType.equals(LockType.X) || currLockType.equals(LockType.S)) {
            return;
        } else {
            List<ResourceName> releaseLocks = getDescendants(transaction);
            boolean onlySIS = true;

            if (currLockType.equals(LockType.X) || currLockType.equals(LockType.IX) || currLockType.equals(LockType.SIX)) {
                onlySIS = false;
            }

            for (ResourceName releaseLock : releaseLocks) {
                LockType type = this.lockman.getLockType(transaction, releaseLock);
                if (type.equals(LockType.X) || type.equals(LockType.IX) || type.equals(LockType.SIX)) {
                    onlySIS = false;
                }
            }

            this.updateNumChildsLocksDescendants(transaction, -1);

            releaseLocks.add(this.name);

            if (onlySIS) {
                this.lockman.acquireAndRelease(transaction, this.name, LockType.S, releaseLocks); //add to queue
            } else {
                this.lockman.acquireAndRelease(transaction, this.name, LockType.X, releaseLocks); //add to queue
            }
        }
    }
    ...
    private List<ResourceName> getDescendants(TransactionContext transaction) {
        ...
        for (LockContext descendantContext : this.children.values()) {
            sisDesc.addAll(descendantContext.sisDescendants(transaction));
            LockType currDescendantLockType = this.lockman.getLockType(transaction, descendantContext.getResourceName());
            if (!currDescendantLockType.equals(LockType.NL)) {
                sisDesc.add(descendantContext.getResourceName());
            }
        }
    ...
    private void updateNumChildsLocksDescendants(TransactionContext transaction, int num) { // DFS
        for (LockContext descendantContext : this.children.values()) {
            LockType currDescendantLockType = this.lockman.getLockType(transaction, descendantContext.getResourceName());
            if (!currDescendantLockType.equals(LockType.NL)) {
                descendantContext.updateNumChildLocks(transaction.getTransNum(), num);
            ...
            descendantContext.updateNumChildsLocksDescendants(transaction, num);
        }
    }
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        // TODO(proj4_part2): implement
        LockType effectiveLockType = this.lockman.getLockType(transaction, this.name);
        if (!effectiveLockType.equals(LockType.NL)) {
            return effectiveLockType;
        }

        ResourceName tmpName = this.name.parent();;

        while (tmpName != null) {
            effectiveLockType = this.lockman.getLockType(transaction, tmpName);
            if (effectiveLockType.equals(LockType.S) || effectiveLockType.equals(LockType.X)) {
                return effectiveLockType;
            } else if (effectiveLockType.equals(LockType.SIX)) {
                return LockType.S;
            }
            tmpName = tmpName.parent();
        }
    ...
    private boolean hasSIXAncestor(TransactionContext transaction) {
        // TODO(proj4_part2): implement
        LockContext ancestorContext = this.parentContext();

        while (ancestorContext != null) {
            LockType ancestorLockType = this.lockman.getLockType(transaction, ancestorContext.getResourceName());
