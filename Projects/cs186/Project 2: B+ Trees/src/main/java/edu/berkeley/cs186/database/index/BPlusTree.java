...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (opt.isPresent()) {
            Pair<DataBox, Long> returnedVal = opt.get();

            List<DataBox> newRootKeys = new ArrayList<>();
            newRootKeys.add(returnedVal.getFirst());

            List<Long> newRootChirdren = new ArrayList<>();
            newRootChirdren.add(root.getPage().getPageNum());
            newRootChirdren.add(returnedVal.getSecond());

            BPlusNode newRoot = new InnerNode(this.metadata, this.bufferManager, newRootKeys, newRootChirdren, this.lockContext);
            updateRoot(newRoot);
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(LeafNode leaf, Iterator<RecordId> iter) {
            this.leaf = leaf;
            this.iter = iter;
        ...
        public RecordId next() {
            ...
            RecordId nextRecordID = iter.next();
