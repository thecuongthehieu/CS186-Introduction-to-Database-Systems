...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        pass1Map.put(new HashSet(Collections.singleton(this.startTableName)), minCostSingleAccess(this.startTableName));
        for (String joinTableName : this.joinTableNames) {
            pass1Map.put(new HashSet(Collections.singleton(joinTableName)), minCostSingleAccess(joinTableName));
        }

        Map<Set, QueryOperator> prevMap = new HashMap<>(pass1Map);
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        int exceptIndex  = this.selectColumnNames.size();
        List<Integer> indexColumns  = this.getEligibleIndexColumns(table);
        for (Integer indexCol : indexColumns) {
            String columnName = this.selectColumnNames.get(indexCol);
            PredicateOperator predicate = this.selectOperators.get(indexCol);
            DataBox value = this.selectDataBoxes.get(indexCol);
            QueryOperator indexScanOnThisColumn = new IndexScanOperator(this.transaction, table, columnName, predicate, value);

            int currCost = indexScanOnThisColumn.estimateIOCost();
            if (currCost < minCost) {
                minOp = indexScanOnThisColumn;
                minCost = currCost;
                exceptIndex = indexCol;
            }
        }
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
         ...
         */
        for (Set joinedTables : prevMap.keySet()) {
            QueryOperator leftOperator = prevMap.get(joinedTables);
            ...
            for (int i = 0; i < this.joinTableNames.size(); ++i) {
                ...
                //joinLeftColumnNames have format: tableName.ColumnName
                String[] leftSide = this.getJoinLeftColumnNameByIndex(i);
                String[] rightSide = this.getJoinRightColumnNameByIndex(i);

                if (joinedTables.contains(leftSide[0]) && !joinedTables.contains(rightSide[0])) {
                    newSet.add(rightSide[0]);
                    rightOperator = pass1Map.get(new HashSet(Collections.singleton(rightSide[0])));

                    leftColumnName = leftSide[1];
                    rightColumnName = rightSide[1];

                } else if (joinedTables.contains(rightSide[0]) && !joinedTables.contains(leftSide[0])) {
                    newSet.add(leftSide[0]);
                    rightOperator = pass1Map.get(new HashSet(Collections.singleton(leftSide[0])));

                    leftColumnName = rightSide[1];
                    rightColumnName = leftSide[1];
                } else {
                    continue;
                }

                QueryOperator minCostJoin = this.minCostJoinType(leftOperator, rightOperator, leftColumnName, rightColumnName);

                newSet.addAll(joinedTables);

                if (!map.containsKey(newSet)) {
                    map.put(newSet, minCostJoin);
                } else {
                    if (map.get(newSet).estimateIOCost() > minCostJoin.estimateIOCost()) {
                        map.put(newSet, minCostJoin);
                    }
                }
            }
