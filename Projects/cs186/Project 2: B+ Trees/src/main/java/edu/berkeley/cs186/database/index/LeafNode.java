...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (this.keys.contains(key)) {
            String msg = String.format(
                    "B+ trees do not support duplicate entries with the same key");
            throw new BPlusTreeException(msg);
        }

        int insertID = InnerNode.numLessThan(key, this.keys);
        this.keys.add(insertID, key);
        this.rids.add(insertID, rid);
        ...
        if (this.keys.size() == (2 * d + 1)) {
            ...
            // Create right node.
            BPlusNode newLeaf = new LeafNode(this.metadata, this.bufferManager, rightKeys, rightRids, this.rightSibling, this.treeContext);
            ...
            return Optional.of(new Pair<DataBox, Long>(rightKeys.get(0), newLeaf.getPage().getPageNum()));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            ...
            this.keys.add(newData.getFirst());
            this.rids.add(newData.getSecond());

            int d = this.metadata.getOrder();
            int f = (int) Math.ceil(2 * d * fillFactor);
            if (this.keys.size() == (f + 1)) {
                List<DataBox> leftKeys = keys.subList(0, f);
                List<DataBox> rightKeys = keys.subList(f, f + 1);
                List<RecordId> leftRids  = rids.subList(0, f);
                List<RecordId> rightRids  = rids.subList(f, f + 1);
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSibling = (tmp == -1) ? Optional.empty() : Optional.of(tmp);
        int n = buf.getInt();
        for (int i = 0; i < n; ++i) {
