...
public class SortOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        List<Iterator<Record>> iteratorOfRuns = new ArrayList<>();

        for (int i = 0; i < n; ++i) {
            Iterator<Record> tempIterator = runs.get(i).iterator();
            if (tempIterator.hasNext()) {
                Pair<Record, Integer> item = new Pair<>(tempIterator.next(), new Integer(i));
                pQueue.add(item);
            }
            iteratorOfRuns.add(tempIterator);
        }

        while (!pQueue.isEmpty()) {
            Pair<Record, Integer> minItem = pQueue.poll();
            mergedRun.addRecord(minItem.getFirst().getValues());
            int runID = minItem.getSecond().intValue();
            if (iteratorOfRuns.get(runID).hasNext()) {
                pQueue.add(new Pair<>(iteratorOfRuns.get(runID).next(), new Integer(runID)));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int numBatch = N / (this.numBuffers - 1);
        for (int i = 0; i < numBatch; ++i) {
            mergedRuns.add(mergeSortedRuns(runs.subList(i * (this.numBuffers - 1), (i + 1) * (this.numBuffers - 1))));
        }
        if (numBatch * (this.numBuffers - 1) < N) {
            mergedRuns.add(mergeSortedRuns(runs.subList(numBatch * (this.numBuffers - 1), runs.size())));
        }
    ...
    public String sort() {
        ...
        while (pageIterator.hasNext()) {
            BacktrackingIterator<Record> recordIterator = this.transaction.getBlockIterator(this.tableName, pageIterator, this.numBuffers);
            Run BsizeRun = createRunFromIterator(recordIterator);

            // Conquer Pass
            Run BsizeSortedRun = sortRun(BsizeRun);
            sortedRuns.add(BsizeSortedRun);
        }
